...
public class Natural {
    ...
    public static int leadingDigit(int[] digits) {
        ...
        // At this point in the code, we know that D[i+1], ..., D[n-1] are all zero, and that D[i] != 0 && i = 0,
        // since the while loop has been terminated at this point.

        // This implies the postcondition below, since they are the same statements.
    ...
    public Natural toBase(int base) {
        ...
        // The postcondition holds at the end because r.value becomes D[0] b^0 + D[1] b^1 + ... + D[n-1] b^n-1,
        // due to each iteration, multiplying the base to r and then adding D[i] to it. Because this.value is
        // equivalent to D[0] b^0 + D[1] b^1 + ... + D[n-1] b^n-1, the postcondition holds
    ...
    public String toString() {
        ...
        // This implies the postcondition below, since if you plug in i's value to buf's equation, you get that
        // buf = buf = ch(D[n-1]), ch(D[n-2]), ..., ch(D[0])
    ...
    public Natural plus(Natural other) {
        ...
        // Takes the sum of both A and B in each index until i = other.digits.length
        ...
        // Given the precondition from natural numbers, the digits are between 0 and b - 1. Since D[0] = A[0] + B[0],
        // D[0] = 2(b-1) = 2b - 2 which is less than 2b - 1. Since 2b - 2 is weaker than 2b - 1, it follows that the Inv
        // implies the post-condition.

        // The first part is given from the end of the previous post-condition (invariant). The second part holds because
        // the preconditions are that i = other.digits.length and that this.digits.length > other.digits.length. It
        // combining these facts, it follows that B[k] = other.digits[k] if k < other.digits.length and
        // B[k] = 0 otherwise

        // Adds the remaining extra digits into the new digits
        // Inv: D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[i-1] = A[i-1]+B[i-1],
        ...
        // We have this because the post-condition of value is val = D[0] + D[1] b + D[2] b^2 + ... + D[n-1] b^{n-1}.
        // So, since this.value = A[A[0] + A[1] b + A[2] b^2 + ... + A[n-1] b^{n-1}],
        // other.value = B[B[0] + B[1] b + B[2] b^2 + ... + B[n-1] b^{n-1}, summing them will get
        // D[0] + D[1] b + ... + D[n-1] b^{n-1} based on the precondition above
        ...
        // now, it is true that both
        // this.value + other.value = D[0] + D[1] b + ... + D[n] b^{n} and D[0] < b, D[1] < b, ..., D[n-1] < b.
        // where n = this.digits.length, and that D[n] < b
        // (1) The postcondition holds because newDigits = D[0] + D[1] b + ... + D[n] b^{n} = this.value + other.value
        // (2) The preconditions of this constructor hold since 2 <= this.base <= 36 alomg with the fact that each of
        // the digits in newDigits are smaller than the base.
